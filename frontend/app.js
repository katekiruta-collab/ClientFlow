...
const API_BASE =
    "https://clientflow-production-59b4.up.railway.app/api";
...
if (telegramApp) {
    ...
    window.haptic = function (type = "light") {
        if (
            telegramApp.HapticFeedback?.impactOccurred
        ) {
...
function normalizeInvoice(invoice) {
    ...
    return {
        ...
        appointmentId:
            ...
            invoice.appointment_id !== undefined
                ? Number(
...
function findInvoiceForAppointment(
    appointmentId
) {
    return clientFlowData.invoices.find(
        invoice =>
...
function renderDashboard() {
    ...
    nextCard.innerHTML = `
        ...
        <button
            class="primary-btn"
            type="button"
            data-dashboard-open="appointments"
        >
            Открыть записи
        </button>
    `;
...
async function updateClient(
    ...
    const index =
        clientFlowData.clients.findIndex(
            client =>
                Number(
                    client.id
                ) === Number(id)
...
async function createAppointment(
    ...
    clientFlowData.appointments.push(
        created
    );
...
async function updateAppointment(
    ...
    const index =
        clientFlowData.appointments.findIndex(
            appointment =>
                Number(
                    appointment.id
                ) === Number(id)
...
function openAppointmentEditForm(
    id
) {
    const appointment =
        findAppointment(id);
...
async function updateInvoice(
    ...
    const index =
        clientFlowData.invoices.findIndex(
            invoice =>
                Number(
                    invoice.id
                ) === Number(id)
            ...
            card.innerHTML = `
                <div class="invoice-main">
                    ...
                    <div class="card-actions">
                        ...
                        <button
                            class="small-btn invoice-status-btn"
                            type="button"
...
document.addEventListener(
    ...
    function (event) {
        ...
        if (
            ...
            confirmDelete(
                "Удалить клиента?",
                "Клиент будет удалён окончательно.",
                () =>
                    deleteClient(id)
            );
        }
        ...
        if (
            ...
            confirmDelete(
                "Удалить запись?",
                "Запись будет удалена окончательно.",
                () =>
                    deleteAppointment(
                        id
                    )
        ...
        if (
            ...
            confirmDelete(
                "Удалить счёт?",
                "Счёт будет удалён окончательно.",
                () =>
                    deleteInvoice(
                        id
                    )
...
async function initializeClientFlow() {
    ...
    try {
        ...
        console.log(
            "ClientFlow initialized successfully"
        );

        console.log(
            "Clients:",
            clientFlowData.clients.length
        );

        console.log(
            "Appointments:",
            clientFlowData.appointments.length
        );

        console.log(
            "Invoices:",
            clientFlowData.invoices.length
        );
    } catch (error) {
        console.error(
            "ClientFlow initialization failed:",
            error
        );

        showError(
            error.message ||
                "Не удалось загрузить ClientFlow"
        );
    }
}
